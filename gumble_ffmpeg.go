 ...
 * Gumble FFMPEG
 * By Tim Cooper
 * Copyright (c) 2014, 2015 Tim Cooper (MIT License)
 */
...
func (s *Stream) Play(file string, time string, callbacks... func()) error {
	s.playLock.Lock()
	...
	cmd := exec.Command(s.Command, "-i", file, "-ac", "1", "-ss", time, "-ar", strconv.Itoa(gumble.AudioSampleRate), "-f", "s16le", "-")
